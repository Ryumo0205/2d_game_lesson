...
@onready var ui_counter : Label = get_tree().current_scene.get_node("./UI_Layer/MarginContainer/Label2")
...
func check_hp():
	...
	if current_hp <= 0:
		ui_counter.died_enemy = ui_counter.died_enemy + 1
		ui_counter.text = str(ui_counter.died_enemy)
