...
func _ready() -> void:
    ...
    ExpManager.bonus_bullet.connect(on_bonus_bullet)
    $bonus_timer.timeout.connect(on_bonus_timeout)
...
func shoot_bullet() -> void:
    ...
    if is_bonus_bullet == true:
        bullet_instance.is_bonus = true
    else:
        bullet_instance.is_bonus = false
...
func weapon_update(_level):
    ...
    shoot_rate = shoot_rate - 0.02

func on_bonus_bullet():
    is_bonus_bullet = true
    $bonus_timer.start()
    
func on_bonus_timeout():
    is_bonus_bullet = false
